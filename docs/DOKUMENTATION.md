...
| `kicad-v2/G3VM_601AY2.pretty`+`.3dshapes`, `kicad-v2/ESP32-C3-SuperMini.pretty`+`.3dshapes` | projektlokale Footprints (aus dem Board extrahiert) **inkl. STEP-Modellen** (A-DIP4_THD_OMR.step, ESP32-C3 SuperMini STEP); Modellpfade projektrelativ `${KIPRJMOD}` | Quelle |
...
| **v2.0.1 2026-08-12** | SW4/X4 entfernt (Shelly/Snubber extern); **J3–J6** als OLED-Reserve-Header rechts neben J2 (Nutzer, GUI); F1 gedreht/versetzt; L_F südlich um K1 (B.Cu-Unterführung) → **K1-SELV in der GND-Fläche**; Display in Firmware `rotation: 180°`; **Dom-Schlitze von Ø10 auf Ø11** (die 2 rechten asymmetrisch: innen/Boden +0,5 mm, Außenwand liegt an der Board-Kante — dort ist kein Material); DRC ohne echte Fehler. |
