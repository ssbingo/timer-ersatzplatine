...
| 2026-07-16 | Roaming-Haken persistent + sichtbar + Status-Zeile | Bug: ESPHome-`save()` legt auf ESP32 nur in RAM-Warteschlange (`s_pending_save`) → Haken „verschwand" nach Reconnect/Neustart. Fix: `netcfg_save()` ruft `global_preferences->sync()` (sofort ins Flash, gilt für alle Einstellungen). Zusätzlich: `roaming` in `/api/status`, der Haken wird laufend daraus gesetzt (self-healing, mit 4-s-Hold nach manueller Änderung) und bleibt nach dem Neuverbinden sichtbar; neue Status-Zeile „WLAN-Roaming (802.11k/v): EIN/AUS". `esphome compile` grün |
